...
export function DossiersPage() {
  ...
  const [selectedContactsByDossier, setSelectedContactsByDossier] = useState<Record<string, number | null>>({});
    ...
    <>
      ...
      <section className="grid gap-4">
        {data.dossiers.map((dossier) => {
          const selectedContactId = selectedContactsByDossier[dossier.key] ?? null;
          const filteredDocuments = selectedContactId
            ? dossier.documents.filter((item) =>
                item.contact?.id === selectedContactId || item.linkedContacts.some((contact) => contact.id === selectedContactId),
              )
            : dossier.documents;
          const filteredObligations = selectedContactId
            ? dossier.obligations.filter((item) => item.contact?.id === selectedContactId)
            : dossier.obligations;
          const selectedContact = selectedContactId
            ? dossier.contacts.find((item) => item.id === selectedContactId) ?? null
            : null;
                  ...
                  <div>
                    ...
                    {selectedContact ? (
                      <div className="mt-1 text-xs text-stone-600">
                        Filter actief op contact: <span className="font-semibold text-ink-900">{selectedContact.name}</span>
                      </div>
                    ) : null}
              ...
              >
                <div className="rounded-[1.35rem] bg-sand-50/78 p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {selectedContact ? (
                      <button
                        className="text-xs font-medium text-pine-700 transition hover:text-pine-800"
                        type="button"
                        onClick={() =>
                          setSelectedContactsByDossier((current) => ({
                            ...current,
                            [dossier.key]: null,
                          }))
                        }
                        ...
                        >
                          <button
                            className={[
                              ...
                              isSelected ? "text-pine-900" : "",
                            ].join(" ")}
                            type="button"
                            onClick={() =>
                              setSelectedContactsByDossier((current) => ({
                                ...current,
                                [dossier.key]: current[dossier.key] === item.id ? null : item.id,
                              }))
                            }
                          >
                            ...
                            {isSelected ? <div className="mt-1 text-xs font-medium text-pine-700">Filter actief</div> : null}
                          </button>
                        </div>
                      );
                ...
                <div className="rounded-[1.35rem] bg-sand-50/78 p-4">
                  ...
                  <div className="mt-3 max-h-[28rem] space-y-2.5 overflow-y-auto pr-1">
                    ...
                    )) : <p className="text-sm text-stone-500">{selectedContact ? "Geen documenten voor dit contact." : "Geen documenten in dit dossier."}</p>}
                  </div>
                </div>
                ...
                <div className="rounded-[1.35rem] bg-sand-50/78 p-4">
                  ...
                  <div className="mt-3 max-h-[28rem] space-y-2.5 overflow-y-auto pr-1">
                    {filteredObligations.length ? filteredObligations.map((item) => (
                      <div
                        className="rounded-[1.35rem] bg-white/90 px-3.5 py-2.5 shadow-[0_8px_18px_rgba(29,28,23,0.04)]"
                        draggable
                        key={item.id}
                    ...
                    )) : <p className="text-sm text-stone-500">{selectedContact ? "Geen verplichtingen voor dit contact." : "Geen verplichtingen in dit dossier."}</p>}
                  </div>
                </div>
              </CollapsibleSection>
            </div>
